...
### C1. SQL Injection Risk via `literal_column` with f-string
...
### C2. `traverse_graph` Returns Dict Instead of Tuple — Interface Violation
**File:** `src/infrastructure/database/repositories/dependency_repository.py`
...
### C3. `mark_stale_edges` Task Passes Wrong Keyword Argument
...
### C4. `CircularDependencyDetector` Is Stateful and Not Reusable
...
### C5. Recursive `_strongconnect` Will Stack-Overflow on Deep Graphs
**File:** `src/domain/services/circular_dependency_detector.py`
...
### C6. No Transactional Boundary in Ingestion Use Case
...
### C7. Auth Middleware Double-Commit and Separate Session
...
### C8. CORS Wildcard with Credentials Is Browser-Invalid
...
### C9. `_fetch_services` Accesses `model.metadata` Instead of `model.metadata_`
**File:** `src/infrastructure/database/repositories/dependency_repository.py`
...
### C10. Recursive CTE Lacks Cycle Prevention in WHERE Clause
**File:** `src/infrastructure/database/repositories/dependency_repository.py`
...
### C11. `IngestDependencyGraphUseCase` Constructor Mismatch in OTel Task
...
**I7. Upstream/downstream statistics calculation is incorrect**
...
**I13. `detect_cycles` is unnecessarily `async`**
...
**I16. Dead code in ingestion use case**
...
2. **Full adjacency list loaded into memory** for Tarjan's — for 5,000+ services with 50,000+ edges, this could consume significant memory
3. **Missing CTE cycle prevention** (C10) means cyclic graphs could cause exponential query explosion
...
2. **E2E conftest defaults to development database** — local runs without `DATABASE_URL` set will corrupt dev data
...
1. **Fix all 11 critical issues** — these represent runtime crashes, security risks, or data integrity violations
2. **Address the important issues**, prioritizing:
   - I4/I5 (rate limiting is non-functional)
   - I3 (FK constraints)
   - I7 (statistics correctness)
...
5. **Fix E2E test infrastructure** to resolve the remaining 12 failing tests
